...
export async function fetchCoursesFromSanity(): Promise<Course[]> {
  const docs = await sanityClient.fetch<Parameters<typeof mapSanityCourseToCourse>[0][]>(
    coursesQuery
  );
...
export async function fetchCourseBySlugFromSanity(slug: string): Promise<Course | null> {
