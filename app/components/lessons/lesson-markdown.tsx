...
function getFilenameFromClass(className?: string): string | undefined {
  ...
  return match?.[2] ?? (lang === "ts" || lang === "typescript" ? "example.ts" : lang === "rust" ? "example.rs" : `example.${lang}`);
}
...
const markdownComponents = {
  ...
  code: ({ className, children, ...props }: React.HTMLAttributes<HTMLElement>) => {
    ...
    return (
      <code
        className={cn("rounded bg-muted/60 px-1.5 py-0.5 font-mono text-sm", className)}
        {...props}
      >
        {children}
      </code>
  ...
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 className="mt-6 border-b border-border pb-2 font-semibold text-lg first:mt-0">
      {children}
    </h2>
  ),
